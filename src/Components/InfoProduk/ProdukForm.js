...
const ProdukForm = () => {
	...
	useEffect(() => {
		axios.
			get("https://finalsecondhand-staging.herokuapp.com/categories")
			.then((res) =>{
				console.log(res);
		setOptions(res);
	})
}, ["https://finalsecondhand-staging.herokuapp.com/categories"])
...
return (
	<div className="col-12 col-lg-4 produk_form_container">
		<form className="infoProdukForm_container" onSubmit={handleSubmit(formSubmithandler)}>
			...
			<div>
				<select type="text" name=" produk_desc" id="produk_categories" required placeholder="Pilihan Produk" className="infoProduk_box" {...register("produk_categories")}>
			...
			<div>
				<input type="text" name=" produk_categories" id="produk_desc" required placeholder="Contoh Deskripsi" className="infoProduk_box infoProduk_lastForm" {...register("produk_desc")}></input>
			</div>
			<div>
